...
def calc_prop_logP(unseen_smile_ls, property_name, props_collect):
    ...
    for smile in unseen_smile_ls:
        ...
        if did_convert:  # ensure valid smile
            ...
            raise Exception(
                "Invalid smile encountered while atempting to calculate logP"
            )
...
def calc_prop_SAS(unseen_smile_ls, property_name, props_collect):
    ...
    for smile in unseen_smile_ls:
        mol, smi_canon, did_convert = evo.sanitize_smiles(smile)
        if did_convert:  # ensure valid smile
            props_collect[property_name][smile] = calculate_score(mol)
        else:
            raise Exception(
                "Invalid smile encountered while atempting to calculate SAS"
            )
...
def calc_prop_RingP(unseen_smile_ls, property_name, props_collect):
    ...
    for smi in unseen_smile_ls:
        ...
        if did_convert:
            ...
            raise Exception(
                "Invalid smile encountered while atempting to calculate Ring penalty"
            )
...
def calc_prop_SIMIL(starting_smile, unseen_smile_ls, property_name, props_collect):
    ...
    for smile in unseen_smile_ls:
        mol, smi_canon, did_convert = evo.sanitize_smiles(smile)
        if did_convert:  # ensure valid smile
            props_collect[property_name][smile] = evo.molecule_similarity(
                mol, target
            )  # Add calculation
        else:
            raise Exception(
                "Invalid smile encountered while atempting to calculate SIMILARITY: ",
                smile,
            )
...
def fitness(
    ...
    if properties_calc_ls == None:
        ...
        ) = obtained_standardized_properties(
            molecules_here, logP_results, SAS_results, ringP_results, similar_results
        )
        ...
        writer.add_scalar(
            "Mean Similarty", similarity_calculated.mean(), generation_index
        )  # Mean similarity
        writer.add_scalar(
            "Max  Similarty", max(similarity_calculated), generation_index
        )  # Max similarity

        similarity_calculated = np.array(
            [0 if x > desired_delta else -(10 ** 6) for x in similarity_calculated]
        )
        ...
        writer.add_scalar(
            "non standr mean logp", logP_calculated.mean(), generation_index
        )
        writer.add_scalar(
            "non standr min sas", min(SAS_calculated), generation_index
        )  # SAS plots
        writer.add_scalar(
            "non standr mean sas", SAS_calculated.mean(), generation_index
        )
        writer.add_scalar(
            "non standr min ringp", min(RingP_calculated), generation_index
        )  # RingP plots
        writer.add_scalar(
            "non standr mean ringp", RingP_calculated.mean(), generation_index
        )

        # max logP - non standardized
        with open("{}/max_logp.txt".format(data_dir), "a+") as handle:
            handle.write(str(max(logP_calculated)) + "\n")

        # mean logP - non standardized
        with open("{}/avg_logp.txt".format(data_dir), "a+") as handle:
            handle.write(str(logP_calculated.mean()) + "\n")

        # min SAS - non standardized
        with open("{}/min_SAS.txt".format(data_dir), "a+") as handle:
            handle.write(str(min(SAS_calculated)) + "\n")

        # mean SAS - non standardized
        with open("{}/avg_SAS.txt".format(data_dir), "a+") as handle:
            handle.write(str(SAS_calculated.mean()) + "\n")

        # min RingP - non standardized
        with open("{}/min_RingP.txt".format(data_dir), "a+") as handle:
            handle.write(str(min(RingP_calculated)) + "\n")

        # mean RingP - non standardized
        with open("{}/avg_RingP.txt".format(data_dir), "a+") as handle:
            handle.write(str(RingP_calculated.mean()) + "\n")
...
def obtain_fitness(
    ...
    writer.add_scalar(
        ...
        np.array(discriminator_predictions).mean(),
        generation_index,
    )
    writer.add_scalar(
        "max discriminator score", max(discriminator_predictions), generation_index
    )
    with open("{}/avg_discr_score.txt".format(data_dir), "a+") as handle:
        handle.write(str(np.array(discriminator_predictions).mean()) + "\n")

    with open("{}/max_discr_score.txt".format(data_dir), "a+") as handle:
        handle.write(str(max(discriminator_predictions)[0]) + "\n")
    ...
    with open("{}/best_in_generations.txt".format(data_dir), "a+") as handle:
        best_gen_str = "index: {},  smile: {}, fitness: {}, logP: {}, sas: {}, ringP: {}, discrm: {}".format(
            generation_index,
            smiles_ordered[0],
            fitness_ordered[0],
            logP_calculated[0],
