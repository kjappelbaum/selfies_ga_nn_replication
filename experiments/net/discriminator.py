...
def obtain_initial_discriminator(disc_enc_type, disc_layers, max_molecules_len, device):
    ...
    if disc_enc_type == "smiles" or disc_enc_type == "selfies":
        ...
        discriminator, d_optimizer, d_loss_func = create_discriminator(
            one_hot_len, disc_layers, 0.0, device
        )
        return discriminator, d_optimizer, d_loss_func
    elif disc_enc_type == "properties_rdkit":
        discriminator, d_optimizer, d_loss_func = create_discriminator(
            51, disc_layers, device
        )
        return discriminator, d_optimizer, d_loss_func
...
def do_x_training_steps(
    ...
    for t in range(steps):
        ...
        print(
            "-- Training of the discriminator, num_Updates[ %i / %i]: Loss %.5f"
            % (t + 1, steps, loss.item())
        )
    return net
