...
def create_100_mol_image(
    mol_list, file_name, fitness, logP, SAS, RingCount, discr_scores
):
...
def get_num_bond_types(mol):
    ...
    if num_bonds == 0:
        ...
        return [
            num_single / num_bonds,
            num_double / num_bonds,
            num_triple / num_bonds,
            num_aromatic / num_bonds,
        ]
...
def count_conseq_double(mol):
    ...
    for b in bonds:
        ...
        if (
            ...
            and curr_BType == rdkit.Chem.rdchem.BondType.DOUBLE
        ):
            count_conseq_doub += 1
        previous_BType = curr_BType
...
def get_mol_info(smi):
    ...
    if (
        num_carbon == 0
    ):  # Avoid division by zero error, set num_carbon to a very small value
        num_carbon = 0.0001

    basic_props = [
        num_atoms / num_carbon,
        num_hydro / num_carbon,
        num_nitro / num_carbon,
        num_sulphur / num_carbon,
        num_oxy / num_carbon,
        num_clorine / num_carbon,
        num_bromine / num_carbon,
        num_florine / num_carbon,
    ]
...
def obtain_discr_encoding(
    molecules_here, disc_enc_type, max_molecules_len, num_processors, generation_index
):
    ...
    if disc_enc_type == "smiles":
        ...
        ratio = (
            len(molecules_here_unique) / num_processors
        )  # number of smiles each process shall handle
