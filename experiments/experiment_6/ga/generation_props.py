...
def obtain_discrm_data(
    ...
    if disc_enc_type == "smiles":
        ...
        # Flipped compared to original implementation (!)
        dataset_y = np.array(
            [0 if x in molecules_reference else 1 for x in smiles_mutated]
            + [0 for i in range(len(dataset_smiles) - len(smiles_mutated))]
        )
